...
(function () {
    ...
    // Store file objects and their object URLs
    const fileObjects = new Map(); // pathName -> File object
    const objectURLs = new Map(); // pathName -> object URL
...
::selection {
    ...
    class Reader {
            ...
            closeElement.addEventListener('click', event => {
                ...
                if (that._tabs.length === 1) {
                    ...
                    if (objectURLs.has(pathName)) {
                        URL.revokeObjectURL(objectURLs.get(pathName));
                        objectURLs.delete(pathName);
                        fileObjects.delete(pathName);
                    }
                ...
                // Clean up object URL
                const pathName = that._paths[positionToRemove];
                if (objectURLs.has(pathName)) {
                    URL.revokeObjectURL(objectURLs.get(pathName));
                    objectURLs.delete(pathName);
                    fileObjects.delete(pathName);
        ...
        _getTabLabel(pathName) {
            ...
            // Handle both file paths and object URLs
            if (pathName.startsWith('blob:')) {
                const file = fileObjects.get(pathName);
                return file ? file.name : 'PDF Document';
            }
        ...
        _openInViewer(pathName) {
            ...
            // Use object URL if available, otherwise use pathName directly
            const url = objectURLs.get(pathName) || pathName;
        ...
        _openFile(fileOrPathName) {
            ...
            if (fileOrPathName instanceof File) {
                const objectURL = URL.createObjectURL(fileOrPathName);
                const pathName = objectURL; // Use object URL as path identifier
                fileObjects.set(pathName, fileOrPathName);
                objectURLs.set(pathName, objectURL);
                this._updateTitle(fileOrPathName.name);
                this._addTab(pathName);
            } else {
                // Handle string path (for tags tab or other cases)
        ...
        _setMenuItemEvents() {
            // Web: Handle file input instead of Electron IPC
        ...
        _setSeekEvents() {
            ...
            this._rightSeekElement.addEventListener('click', event => {
